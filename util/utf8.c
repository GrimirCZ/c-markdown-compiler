...
void *md_nextcodepoint(const void *utf8_restrict str) {
    const char *s = (const char *) str;
...
char *md_cdptostr(const utf8_int32_t *pts) { // TODO: move length detection to caller
    ...
    char *res = calloc(needed_size + 1, sizeof(char)); // plus one for NULL terminator
    char *cur = res;

    for (i = 0; i < charCount; i++) {
        curval = pts[i];
        cur = utf8catcodepoint(cur, curval, needed_size);
